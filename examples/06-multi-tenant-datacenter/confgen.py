...
"""
    ...
    for tenant_id in range(tenants):
        ...
        config.add_link(Link(switch.name, "eth%d" % (tenant_id + 1), r1.name, "eth0"))
        ...
        for server_id in range(servers):
            ...
            config.add_link(
                Link(server.name, "eth0", tenant_sw.name, "eth%d" % (server_id + 1))
            )
    ...
    for update in range(updates):
        ...
        config.add_update(OpenflowUpdate(r1_name, "10.%d.%d.0/24" % (X, Y), "eth1"))
...
def main():
    ...
    parser.add_argument(
        "-u", "--updates", type=int, help="Number of tenants that get updated"
    )
    parser.add_argument(
        "-s", "--servers", type=int, help="Number of servers within each tenant"
    )
    parser.add_argument(
        "-m",
        "--model",
