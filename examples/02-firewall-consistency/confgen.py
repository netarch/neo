...
"""
    ...
    fw1 = DockerNode(
        "fw1", image="kyechou/iptables:latest", working_dir="/", command=["/start.sh"]
    )
    ...
    fw2 = DockerNode(
        "fw2", image="kyechou/iptables:latest", working_dir="/", command=["/start.sh"]
    )
    ...
    for app in range(apps):
        ...
        agg1_sink.add_interface(
            Interface("eth%d" % (2 * app + 2), "9.%d.%d.1/30" % (second, third))
        )
        agg1_sink.add_interface(
            Interface("eth%d" % (2 * app + 3), "9.%d.%d.5/30" % (second, third))
        )
        agg1_source.add_interface(
            Interface("eth%d" % (2 * app + 2), "9.%d.%d.9/30" % (second, third))
        )
        agg1_source.add_interface(
            Interface("eth%d" % (2 * app + 3), "9.%d.%d.13/30" % (second, third))
        )
        agg1_source.add_static_route(
            Route("10.%d.%d.0/24" % (second, third), "9.%d.%d.10" % (second, third))
        )
        agg1_source.add_static_route(
            Route("11.%d.%d.0/24" % (second, third), "9.%d.%d.14" % (second, third))
        )
        agg2_sink.add_interface(
            Interface("eth%d" % (2 * app + 2), "9.%d.%d.17/30" % (second, third))
        )
        agg2_sink.add_interface(
            Interface("eth%d" % (2 * app + 3), "9.%d.%d.21/30" % (second, third))
        )
        agg2_source.add_interface(
            Interface("eth%d" % (2 * app + 2), "9.%d.%d.25/30" % (second, third))
        )
        agg2_source.add_interface(
            Interface("eth%d" % (2 * app + 3), "9.%d.%d.29/30" % (second, third))
        )
        agg2_source.add_static_route(
            Route("10.%d.%d.0/24" % (second, third), "9.%d.%d.26" % (second, third))
        )
        agg2_source.add_static_route(
            Route("11.%d.%d.0/24" % (second, third), "9.%d.%d.30" % (second, third))
        )
        ...
        for host in range(hosts):
            ...
            if host % 2 == 0:  # hosts under access1
                ...
                node.add_interface(
                    Interface("eth0", "10.%d.%d.%d/30" % (second, third, last))
                )
                node.add_static_route(
                    Route("0.0.0.0/0", "10.%d.%d.%d" % (second, third, last - 1))
                )
                config.add_link(
                    Link(node.name, "eth0", access1.name, "eth%d" % acc_intf_num)
                )
                ...
                node.add_interface(
                    Interface("eth0", "11.%d.%d.%d/30" % (second, third, last))
                )
                node.add_static_route(
                    Route("0.0.0.0/0", "11.%d.%d.%d" % (second, third, last - 1))
                )
                config.add_link(
                    Link(node.name, "eth0", access2.name, "eth%d" % acc_intf_num)
                )
...
def main():
    ...
    parser.add_argument("-a", "--apps", type=int, help="Number of applications")
    parser.add_argument(
        "-H", "--hosts", type=int, help="Number of hosts in each application"
    )
    parser.add_argument(
        "-f",
        "--fault",
