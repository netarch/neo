...
for network in "${networks[@]}"; do
    ...
    for emu_pct in 4 8 12 16 20; do
    	for firewall_pct in 20; do
		    for invs in 1 4 8 12 16; do
		        "${CONFGEN[@]}" --topo "$network" -e "$emu_pct" -f "$firewall_pct" -i "$invs" >"$CONF"
		        for procs in 1 4 8 12 16; do
		            for drop in "${DROP_METHODS[@]}"; do
		                name="output.$network_name.$emu_pct-emulated.$firewall_pct-firewalls.$invs-invariants.$procs-procs.$drop"
		                run "$name" "$procs" "$drop" "$CONF" --parallel-invs
		            done
		        done
		        rm "$CONF"
