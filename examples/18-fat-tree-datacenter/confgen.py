...
"""
    ...
    for tenant_id in range(1, k**3 // 4):
        ...
        fw.add_interface(Interface("eth0", "9.%d.%d.6/30" % (X, Y)))  # external
        fw.add_interface(Interface("eth1", "9.%d.%d.9/30" % (X, Y)))  # internal
...
def main():
    ...
    parser.add_argument("-k", "--arity", type=int, required=True, help="Fat-tree arity")
    parser.add_argument(
        "-u",
        "--updates",
        type=int,
